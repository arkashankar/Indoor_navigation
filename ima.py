# -*- coding: utf-8 -*-
"""
Created on Mon Apr 13 10:34:58 2020
...
#img = plt.imread('new_img/frameL01P01.jpg150.jpg')
def img_edge(img):
    ...
    med = np.mean(edges_img)
    edges_img[edges_img>med]=1
    edges_img[edges_img<=med]=0
    return edges_img
...
for _, dirnames, filenames in os.walk(r'D:\Indoor_loc\new_img'):
    count = count+1
for i in filenames:    
    img = plt.imread('D:/Indoor_loc/new_img/'+i)
    pic = img_edge(img) 
    m,n = compare_images(pic,pic2)
    mse_list.append(m)
    ssim_list.append(n)
    
maximus = np.max(ssim_list)
key = ssim_list.index(maximus)
file_found = filenames[key]
